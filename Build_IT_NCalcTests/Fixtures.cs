...
namespace NCalc.Tests
{
    public class Fixtures
    {
        ...
        [Fact]
        public void ShouldFindMissingParameters()
        {
            var e = new Expression("a + b");
            e.AddParameter("a", 1);

            var missing = e.GetMissingParameters();

            Assert.Contains(missing, m => m == "b");
        ...
        public void ShouldFindMissingParametersForMoreComplicatedCase(bool par1, bool par2, bool par3, bool par4, bool par5, bool par6)
        {
            var e = new Expression("Max([Param1] * 7, [Param2]) + [Param3] / if([Param4]>3,[Param5],[Param6])");
            if (par1)
                e.AddParameter("Param1", 1);
            if (par2)
                e.AddParameter("Param2", 5);
            if (par3)
                e.AddParameter("Param3", 5);
            if (par4)
                e.AddParameter("Param4", 2);
            if (par5)
                e.AddParameter("Param5", 1);
            if (par6)
                e.AddParameter("Param6", 2);

            var missing = e.GetMissingParameters();

            if (!par1)
                Assert.Contains(missing, m => m == "Param1");
            if (!par2)
                Assert.Contains(missing, m => m == "Param2");
            if (!par3)
                Assert.Contains(missing, m => m == "Param3");
            if (!par4)
                Assert.Contains(missing, m => m == "Param4");
            if (!par5)
                Assert.Contains(missing, m => m == "Param5");
            if (!par6)
                Assert.Contains(missing, m => m == "Param6");
        }
        ...
        [InlineData("b", true)]
        [InlineData("c", false)]
        public void CheckIfParameterWithNameIsUsed(string parameterName, bool expectedResult)
        {
            var e = new Expression("a + b");
            e.AddParameter("a", 1);

            var result = e.ContainsParameter(parameterName);
